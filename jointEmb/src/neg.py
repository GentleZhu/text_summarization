    ...
    def forward(self, input_labels, out_labels, num_sampled, weights = []):
        ...
        for tp in xrange(len(self.edge_types)):
            ...
            if self.edge_types[tp][2] == 0:
                ...
                if len(weights) > 0:
                    loss = (weight_tensor * log_target_input).sum() + (weight_tensor * log_target_output).sum() + \
                    (weight_tensor.repeat(1, num_sampled).view(-1) * (sum_log_u_noise_v_input + sum_log_u_noise_v_output + \
                        sum_log_u_v_noise_input + sum_log_u_v_noise_output)).sum() / 2
                else:
                ...
                if len(weights) > 0:
                    loss = 2 * (weight_tensor * log_target).sum() + (weight_tensor.repeat(1, num_sampled).view(-1) * (sum_log_u_noise_v +sum_log_u_v_noise)).sum()
                else:
