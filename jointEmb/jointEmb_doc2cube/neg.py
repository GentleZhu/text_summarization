...
class NEG_loss(nn.Module):
    def __init__(self, type_offset, node_types, edge_types, embed_size, pre_train_path, graph_name='', mode=-1,
                 map_mode=-1, weight_decay=0.0, directed=False):
        """
        ...
        self.embed.weight = Parameter(t.FloatTensor(self.num_classes, self.embed_size).uniform_(-0.1, 0.1).cuda())

        if len(pre_train_path) > 0:
            self.embed.weight.data.copy_(t.from_numpy(pre_train_path))
    ...
    def edge_rep(self, input_a, input_b):
        ...
        if self.mode == 1:
            ...
            return t.bmm(input_a.unsqueeze(2), input_b.unsqueeze(1)).view(-1, self.embed_size ** 2) + t.bmm(
                input_b.unsqueeze(2), input_a.unsqueeze(1)).view(-1, self.embed_size ** 2)
        elif self.mode == 3:
    ...
    def forward(self, input_labels, out_labels, num_sampled, weights=[]):
        """
        ...
        :param num_sampled: An int. The number of sampled from noise examples
        :param weights: Tensor with shape of [batch_size] of Long type
        ...
        window_size -= 1
        # sub_batches = []
        for tp in range(len(self.edge_types)):
            loss = 0.0
            reg_loss = 0.0
            ...
            if len(weights) > 0:
                weight_tensor = t.index_select(weights, 0, indices).contiguous().view(-1).float()
            ...
            u_rep = self.embed(Variable(input_tensor))
            v_rep = self.embed(Variable(output_tensor))

            _v_noise = Variable(t.Tensor(sub_batch_size * window_size, num_sampled).
                                uniform_(0, self.type_offset[type_v + 1] - self.type_offset[type_v] - 1).add_(
                self.type_offset[type_v]).long())
            ...
            v_noise_rep = self.embed(_v_noise).neg()

            # u input_tensor
            # v output_tensor

            log_target = self.edge_map(self.edge_rep(u_rep, v_rep), tp).sum(1).squeeze().clamp(min=-6,
                                                                                                    max=6).sigmoid().log()

            sum_log_u_v_noise = self.edge_map(self.edge_rep(v_noise_rep.view(-1, self.embed_size),
                                                            u_rep.repeat(1, num_sampled).view(-1,
                                                                                                self.embed_size)),
                                              tp).sum(1).squeeze().clamp(min=-6, max=6).sigmoid().log()
            if len(weights) > 0:
                loss = 2 * (weight_tensor * log_target).sum() + (
                weight_tensor.repeat(1, num_sampled).view(-1) * (sum_log_u_v_noise)).sum()
            else:
                loss = 2 * log_target.sum() + sum_log_u_v_noise.sum()
    ...
    def predict(self, inputs, outputs, tp):
        ...
        u_rep = self.in_embed(Variable(inputs))
        v_rep = self.out_embed(Variable(outputs))
        log_target = 0.0
        if self.edge_types[tp][2] == 0:

            u_rep = self.embed(Variable(inputs))
            v_rep = self.embed(Variable(outputs))

            log_target = self.edge_map(self.edge_rep(u_rep, v_rep), tp).sum(1).squeeze().sigmoid() + self.edge_map(
                self.edge_rep(u_rep, v_rep), tp).sum(1).squeeze().sigmoid()
            log_target /= 2
        else:
            log_target = self.edge_map(self.edge_rep(u_rep, v_rep), tp).sum(1).squeeze().sigmoid()
