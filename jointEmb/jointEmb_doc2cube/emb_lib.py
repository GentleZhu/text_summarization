...
class SkipGram(object):
    ...
    def __init__(self, arg):
        ...
        type_offset = pickle.load(open(arg['data_dir'] + arg['graph_name'] + '_offset.p', 'rb'))
        self.neg_loss = neg.NEG_loss(type_offset=type_offset, node_types=arg['node_types'],
        ...
        edge_stats = pickle.load(open(arg['data_dir'] + arg['graph_name'] + '_edge_stat.p', 'rb'))
        # print(edge_stats)
        self.mode = arg['mode']
        ...
        self._params = []
        if self.map_mode != -1:
            self._params = [{'params': self.neg_loss.embed.parameters()}]
            for i in range(len(self.neg_loss.edge_mapping)):
                self._params.append({'params': self.neg_loss.edge_mapping[i].parameters(),
                                     'lr': arg['lr'] * arg['lr_ratio'] * (float(len(self.input))) / (
    ...
    def freeze_embedding(self):
        ...
        for param in self.neg_loss.embed.parameters():
    ...
    def train(self):
        ...
        with open(self.log_dir + 'doc2cube_' + self.graph_name + '_op_' + str(self.mode) +
            ...
            for epoch in range(self.iter):
                ...
                for i, data in enumerate(self.data, 0):
                    if self.weighted:
                        ...
                        loss, pure_loss = self.neg_loss(inputs, labels, self.neg_ratio, weights)
                    else:
                        inputs, labels = data
                ...
                if epoch % self.dump_timer == 0:
                    if self.more_param != 'None':
                        model_path = self.model_dir + 'doc2cube_' + self.graph_name + '_' + str(epoch) + '_op_' + str(
                            self.mode) + \
    ...
    def output(self):
        word_embeddings = self.neg_loss.export_embeddings()
